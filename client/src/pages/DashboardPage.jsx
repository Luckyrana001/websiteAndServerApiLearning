...
function BadgeTile({ badge, index }) {
  return <Box sx={{ display: "flex", minWidth: 0, flexDirection: "column", alignItems: "center", gap: .5, textAlign: "center", textTransform: "capitalize" }}><Box component="img" src={medalImage} alt="Achievement badge" sx={{ width: 36, height: 36, objectFit: "cover", borderRadius: 1.25, filter: `hue-rotate(${index * 38}deg)` }} /><Typography variant="caption" fontWeight={800}>{badge.replaceAll("-", " ")}</Typography></Box>;
}

function LeaderboardTile({ entry, index }) {
  const background = index === 0 ? "#fff5cd" : index === 1 ? "#edf1f7" : index === 2 ? "#fce7d7" : "background.default";
  const rankColor = index === 0 ? "#c28a0d" : index === 1 ? "#71839d" : index === 2 ? "#a85c32" : "#7890b0";
  return <Box sx={{ display: "flex", alignItems: "center", gap: 1.25, minHeight: 52, p: .75, borderRadius: 1.75, bgcolor: background, border: 1, borderColor: "divider", color: "text.primary" }}><Box sx={{ display: "grid", width: 30, height: 30, flex: "0 0 auto", placeItems: "center", borderRadius: "50%", color: "#fff", bgcolor: rankColor, fontWeight: 900 }}>{index + 1}</Box><Box sx={{ flexGrow: 1, minWidth: 0, color: "text.primary" }}><Typography variant="body2" sx={{ display: "block", overflow: "hidden", color: "text.primary !important", fontWeight: 800, whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{String(entry.name || entry.email || "Anonymous player")}</Typography><Typography variant="caption" sx={{ display: "block", color: "text.secondary" }}>Quiz champion</Typography></Box><Typography sx={{ color: "text.primary", flex: "0 0 auto" }} fontWeight={900}>{entry.points ?? 0}<Typography component="span" variant="caption" sx={{ color: "text.secondary" }}> pts</Typography></Typography></Box>;
}
...
function UserDashboard({ data, leaderboard, refresh }) {
  ...
  return <Container maxWidth="xl" sx={{ py: 4, bgcolor: "background.default" }}>
...
function SidePanel({ title, children }) { return <Paper sx={{ p: 2.5, mb: 2, border: 1, borderColor: "divider", boxShadow: 4 }}><Typography variant="overline" color="primary" fontWeight={900}>{title}</Typography><Typography variant="h6" fontWeight={900} gutterBottom>{title === "Leaderboard" ? "Top performers" : title}</Typography><Stack spacing={1}>{children}</Stack></Paper>; }

const emptyQuestion = () => ({ text: "", imageUrl: "", options: "", correct: "" });

function AdminDashboard({ data, refresh }) {
  const [form, setForm] = useState({ title: "", description: "", imageUrl: "", questions: [emptyQuestion()] });
  ...
  function addQuestion() {
    if (form.questions.length < 5) setForm((current) => ({ ...current, questions: [...current.questions, emptyQuestion()] }));
  }

  function removeQuestion(index) {
    if (form.questions.length > 1) setForm((current) => ({ ...current, questions: current.questions.filter((_question, questionIndex) => questionIndex !== index) }));
  }

  async function handleCreate(event) {
    event.preventDefault();
    const questions = form.questions.map((question) => {
      const correctIndexes = question.correct.split(",").map(Number);
      const options = question.options.split(",").map((text) => text.trim()).filter(Boolean).map((text, index) => ({ text, isCorrect: correctIndexes.includes(index + 1) }));
      return { text: question.text, imageUrl: question.imageUrl, options, allowMultiple: options.filter((option) => option.isCorrect).length > 1 };
    });
    await createQuiz({ title: form.title, description: form.description, imageUrl: form.imageUrl, questions });
    setForm({ title: "", description: "", imageUrl: "", questions: [emptyQuestion()] });
    setMessage("Quiz created as inactive. Activate it from the quiz list.");
    refresh();
  }

  return <Container maxWidth="xl" sx={{ py: 4 }}><Typography variant="h4" fontWeight={800} gutterBottom>Admin dashboard</Typography><Grid container spacing={2} sx={{ mb: 3 }}><Grid size={{ xs: 6, md: 3 }}><Metric label="Users" value={data.stats.users} /></Grid><Grid size={{ xs: 6, md: 3 }}><Metric label="Quizzes" value={data.stats.quizzes} /></Grid><Grid size={{ xs: 6, md: 3 }}><Metric label="Active quizzes" value={data.stats.activeQuizzes} color="success.main" /></Grid><Grid size={{ xs: 6, md: 3 }}><Metric label="Completed attempts" value={data.stats.completedAttempts} color="secondary.main" /></Grid></Grid><Grid container spacing={3}><Grid size={{ xs: 12, md: 5 }}><Paper component="form" onSubmit={handleCreate} sx={{ p: 3 }}><Typography variant="h6" fontWeight={800}>Create quiz</Typography><Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>Add between 1 and 5 questions.</Typography><Stack spacing={2}>{message && <Alert severity="success">{message}</Alert>}<TextField label="Quiz title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /><TextField label="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /><TextField label="Quiz image URL (optional)" value={form.imageUrl} onChange={(e) => setForm({ ...form, imageUrl: e.target.value })} />{form.questions.map((question, index) => <Paper variant="outlined" key={index} sx={{ p: 2, bgcolor: "background.default" }}><Stack spacing={2}><Stack direction="row" justifyContent="space-between" alignItems="center"><Typography fontWeight={800}>Question {index + 1}</Typography>{form.questions.length > 1 && <Button type="button" size="small" color="error" onClick={() => removeQuestion(index)}>Remove</Button>}</Stack><TextField label="Question text" value={question.text} onChange={(e) => updateQuestion(index, "text", e.target.value)} required /><TextField label="Question image URL (optional)" value={question.imageUrl} onChange={(e) => updateQuestion(index, "imageUrl", e.target.value)} /><TextField label="Options (comma separated)" helperText="Example: Paris, London, Rome" value={question.options} onChange={(e) => updateQuestion(index, "options", e.target.value)} required /><TextField label="Correct option numbers" helperText="Example: 1 or 1,3 for multiple correct answers" value={question.correct} onChange={(e) => updateQuestion(index, "correct", e.target.value)} required /></Stack></Paper>)}<Button type="button" variant="outlined" onClick={addQuestion} disabled={form.questions.length >= 5}>+ Add question ({form.questions.length}/5)</Button><Button type="submit" variant="contained">Create inactive quiz</Button></Stack></Paper></Grid><Grid size={{ xs: 12, md: 7 }}><Paper sx={{ p: 3 }}><Typography variant="h6" fontWeight={800} gutterBottom>Quiz status</Typography><Stack spacing={1}>{data.quizzes.map((quiz) => <Stack direction="row" alignItems="center" justifyContent="space-between" key={quiz._id}><Box><Typography fontWeight={700}>{quiz.title}</Typography><Typography variant="body2" color="text.secondary">{quiz.questions.length} question(s)</Typography></Box><Select size="small" value={quiz.status} onChange={async (e) => { await updateQuizStatus(quiz._id, e.target.value); refresh(); }}><MenuItem value="active">Active</MenuItem><MenuItem value="inactive">Inactive</MenuItem></Select></Stack>)}</Stack></Paper><Paper sx={{ p: 3, mt: 3 }}><Typography variant="h6" fontWeight={800} gutterBottom>Completed quizzes — allow retake</Typography>{data.completedAttempts?.length ? data.completedAttempts.map((attempt) => <Stack direction="row" alignItems="center" justifyContent="space-between" key={attempt._id} sx={{ py: 1, borderBottom: 1, borderColor: "divider" }}><Box><Typography fontWeight={700}>{attempt.user?.name} — {attempt.quiz?.title}</Typography><Typography variant="body2" color="text.secondary">Score: {attempt.score}%</Typography></Box><Button size="small" variant="outlined" onClick={async () => { await allowQuizRetake(attempt.quiz._id, attempt.user._id); refresh(); }}>Allow retake</Button></Stack>) : <Typography color="text.secondary">No completed attempts.</Typography>}</Paper><Paper sx={{ p: 3, mt: 3 }}><Typography variant="h6" fontWeight={800} gutterBottom>Users</Typography>{data.users.map((entry) => <Stack direction="row" justifyContent="space-between" key={entry._id} sx={{ py: 1, borderBottom: 1, borderColor: "divider" }}><Typography>{entry.name}<br /><small>{entry.email}</small></Typography><Typography>{entry.points} pts</Typography></Stack>)}</Paper></Grid></Grid></Container>;
}
